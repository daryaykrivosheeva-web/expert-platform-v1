...
the repo in Vercel and set **Root Directory** to `Сайт` (this app no longer
lives at the repo root — see [`../README.md`](../README.md) for why), then
deploy.
...
(`Сайт`) as the base directory and deploy.

---

## This folder's layout

This folder is the product — the whole Expert Platform v1 template. It's one
of five top-level folders in the repo; see [`../README.md`](../README.md) for
how the other four (СРМ, Онлайн-запись, Клиенты, Портфолио) fit together.
...
Finished client adaptations' screenshots/video live in `../Портфолио/<Клиент>/`
(one level up), and a client's own brief/materials plus a summary of the work
done live in `../Клиенты/<Клиент>/` — neither is part of the product itself.
