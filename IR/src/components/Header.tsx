  ...
  return (
    ...
    >
      <Container className="flex h-16 items-center justify-between sm:h-[72px]">
        <a
          href="#hero"
          className="font-serif text-lg font-semibold text-ink transition-colors hover:text-bronze sm:text-xl"
        >
          {SITE.name}
        </a>

        <nav className="hidden lg:flex lg:items-center lg:gap-8" aria-label="Основная навигация">
          {NAV.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
            ...
            >
              ...
              {active === item.id && (
                <motion.span
                  ...
                  className="absolute inset-x-0 -bottom-0.5 h-px bg-bronze"
        ...
        <a
          href="#contacts"
          className="hidden rounded-[2px] border border-ink/25 px-5 py-2 text-sm font-medium text-ink transition-colors hover:border-bronze hover:text-bronze lg:inline-flex"
        >
          Связаться
        </a>
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          <nav
            className="flex h-full flex-col gap-1 px-6 pt-8"
          ...
          >
            {NAV.map((item, i) => (
              <motion.a
                ...
                className="border-b border-border py-4 font-serif text-2xl text-ink"
              >
                {item.label}
              </motion.a>
            ))}
